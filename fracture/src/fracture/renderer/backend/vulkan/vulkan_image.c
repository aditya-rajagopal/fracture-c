...
b8 vulkan_image_create(vulkan_context* context,
    ...
    VK_CHECK_RESULT(
        vkAllocateMemory(context->device.logical_device, &alloc_info, context->allocator, &out_image->memory_handle));
    ...
    VK_CHECK_RESULT(
        vkBindImageMemory(context->device.logical_device, out_image->image_handle, out_image->memory_handle, 0));
...
b8 vulkan_image_create_view(vulkan_context* context,
                            vulkan_image* image,
                            VkFormat format,
...
b8 vulkan_image_create_view_from_image(
    vulkan_context* context, VkImage image, VkFormat format, VkImageAspectFlags aspect_flags, VkImageView* out_view) {
