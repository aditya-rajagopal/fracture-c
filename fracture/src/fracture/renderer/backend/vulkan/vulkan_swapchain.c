...
b8 _swapchain_create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* out_swapchain) {
    ...
    create_info.imageExtent =
        extent;  // The extent of the images in the swapchain that we checked 10 times to make sure it is valid
    ...
    create_info.oldSwapchain =
        VK_NULL_HANDLE;  // We are not going to use an old swapchain especially when we are recreating the swapchain

    // Now finally we are going to create the swapchain
    VK_CHECK_RESULT(
        vkCreateSwapchainKHR(context->device.logical_device, &create_info, context->allocator, &out_swapchain->handle));
    ...
    // But for the swapchain, the images are created implicitly when the swapchain is created and it is also the one
    // that destroys the images. So we are going to query the images from the swapchain and then create the image views
    // for them.
    ...
    VK_CHECK_RESULT(vkGetSwapchainImagesKHR(
        context->device.logical_device, out_swapchain->handle, &out_swapchain->image_count, NULL));
    if (!out_swapchain->images) {
        ...
        out_swapchain->images =
            (VkImage*)fr_memory_allocate(sizeof(VkImage) * out_swapchain->image_count, MEMORY_TYPE_RENDERER);
    }
    if (!out_swapchain->image_views) {
        ...
        out_swapchain->image_views =
            (VkImageView*)fr_memory_allocate(sizeof(VkImageView) * out_swapchain->image_count, MEMORY_TYPE_RENDERER);
    }

    VK_CHECK_RESULT(vkGetSwapchainImagesKHR(
        context->device.logical_device, out_swapchain->handle, &out_swapchain->image_count, out_swapchain->images));
    ...
    for (u32 i = 0; i < out_swapchain->image_count; ++i) {
        if (!vulkan_image_create_view_from_image(context,
...
b8 _swapchain_destroy(vulkan_context* context, vulkan_swapchain* swapchain) {
    ...
    // Destroy the image views we dont need to destroy the images as they are implicitly destroyed when the swapchain is
    // destroyed
