...
b8 vulkan_device_query_swapchain_support(
    ...
    if (out_support_info->format_count != 0) {
        if (!out_support_info->formats) {
            out_support_info->formats = fr_memory_allocate(
                sizeof(VkSurfaceFormatKHR) * out_support_info->format_count,
                MEMORY_TYPE_RENDERER);
        }
        VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface, &out_support_info->format_count, out_support_info->formats));
    }

    // Get present modes
    VK_CHECK_RESULT(vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &out_support_info->present_mode_count, 0));
    if (out_support_info->present_mode_count != 0) {
        ...
        VK_CHECK_RESULT(vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &out_support_info->present_mode_count, out_support_info->present_modes));
    }
...
b8 _vulkan_select_physical_device(vulkan_context* context) {
    ...
    for (u32 i = 0; i < physical_device_count; ++i) {
        ...
        if (result) {
            FR_CORE_INFO("Selected physical device: %s", properties.deviceName);
            switch (properties.deviceType) {
                case VK_PHYSICAL_DEVICE_TYPE_OTHER:
                    FR_CORE_INFO("Device type: Other");
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
                    FR_CORE_INFO("Device type: Integrated GPU");
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
                    FR_CORE_INFO("Device type: Discrete GPU");
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU:
                    FR_CORE_INFO("Device type: Virtual GPU");
                    break;
                case VK_PHYSICAL_DEVICE_TYPE_CPU:
                    FR_CORE_INFO("Device type: CPU");
                    break;
                default:
                    FR_CORE_INFO("Device type: Unknown");
                    break;
            }
            FR_CORE_INFO("GPU Driver version: %d.%d.%d",
                         VK_VERSION_MAJOR(properties.driverVersion),
                         VK_VERSION_MINOR(properties.driverVersion),
                         VK_VERSION_PATCH(properties.driverVersion));
            FR_CORE_INFO("API Version: %d.%d.%d",
            ...
            for (u32 j = 0; j < memory.memoryTypeCount; ++j) {
                ...
                if (memory.memoryTypes[j].propertyFlags & VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT) {
                    FR_CORE_INFO("Device local memory: %.2f GB", memory_size_gb);
                } else {
                    FR_CORE_INFO("Shared System memory: %.2f GB", memory_size_gb);
                }
            }
...
b8 _vulkan_physical_device_meets_requirements(
    ...
    if (requirements->discrete_gpu && properties->deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
        FR_CORE_INFO("Device does not meet discrete GPU requirements. Skipping");
        return FALSE;
    }
    ...
    if (
        (!requirements->graphics || (requirements->graphics && out_queue_families->graphics_family_index != UINT32_MAX) ||
        ...
        !requirements->transfer || (requirements->transfer && out_queue_families->transfer_family_index != UINT32_MAX))
    )
    {
        ...
        if (out_swapchain_support->format_count < 1 ||
            ...
            FR_CORE_INFO(
                "Device does not support the swapchain extension. Skipping");
            return FALSE;
        }
        ...
        if (requirements->sampler_anisotropy && !features->samplerAnisotropy) {
            FR_CORE_INFO("Device does not meet anisotropic filtering "
            ...
            for (u64 i = 0; i < required_extension_count; ++i) {
                ...
                if (!found) {
                    fr_memory_free(available_extensions,
                                   ...
                                   MEMORY_TYPE_RENDERER);
                    FR_CORE_INFO("Device: %s does not support the required "
                                 "extension: %s",
                                 properties->deviceName,
                                 requirements->device_extension_names[i]);
                    return FALSE;
