...
FR_FORCE_INLINE f32 fr_sign(f32 val) { return (float)((val > 0.0f) - (val < 0.0f)); }
 ...
 */
FR_FORCE_INLINE f32 fr_step(f32 edge, f32 val) { return val < edge ? 0.0f : 1.0f; }
 ...
 * @return 0 if val < edge, 1 otherwise
 */
FR_FORCE_INLINE f32 fr_step_nobranch(f32 edge, f32 val) { return (f32)(val >= edge); }

FR_FORCE_INLINE f32 fr_smoothinterp(f32 a, f32 b, f32 t) { return fr_lerp(a, b, fr_smooth(t)); }

FR_FORCE_INLINE f32 fr_smoothinterp_clamped(f32 a, f32 b, f32 t) { return fr_lerp(a, b, fr_smooth(fr_clamp_zo(t))); }
