...
FR_FORCE_INLINE void fr_mat4v(const vec4* c0, const vec4* c1, const vec4* c2, const vec4* c3, mat4* m) {
#if FR_SIMD == 1
    m->simd[0] = c0->simd;
    m->simd[1] = c1->simd;
    m->simd[2] = c2->simd;
...
FR_FORCE_INLINE void fr_mat4_random_uniform_custom(
    mat4* m, f32 (*PFN_randf_range)(void* state, f32 min, f32 max), void* state, f32 min, f32 max) {
...
#else
    return fr_equal(a->m00, b->m00) && fr_equal(a->m01, b->m01) && fr_equal(a->m02, b->m02) &&
           fr_equal(a->m03, b->m03) && fr_equal(a->m10, b->m10) && fr_equal(a->m11, b->m11) &&
           fr_equal(a->m12, b->m12) && fr_equal(a->m13, b->m13) && fr_equal(a->m20, b->m20) &&
           fr_equal(a->m21, b->m21) && fr_equal(a->m22, b->m22) && fr_equal(a->m23, b->m23) &&
