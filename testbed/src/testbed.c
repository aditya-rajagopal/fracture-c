...
b8 testbed_initialize(application_handle *app_handle) {
    ...
    for (u32 i = 0; i < 10000000; i++) {
        ...
        // vec3 norm;
        f32 output = fr_vec3_dot(&test, &out);
    }
    ...
    for (u32 i = 0; i < 10000000; i++) {
        ...
        fr_vec3_new_to(fr_random(), fr_random(), fr_random(), &out);
        // vec3 norm;
        f32 output = fr_vec3_dot_simd(&test, &out);
    }
    fr_clock_update(&timer);
    ...
    f32 dot = fr_vec3_dot(&test, &out);
    FR_CORE_TRACE("Dot product of vec3s: %f", dot);
    f32 dot_simd = fr_vec3_dot_simd(&test, &out);
    FR_CORE_TRACE("Dot product of vec3s using SIMD: %f", dot_simd);
    return TRUE;
}
